...
<div class="mx-2 py-6 text-center md:mx-auto md:w-2/3 md:py-10">
  <h1 class="mb-4 text-base font-black leading-4 text-darkblue sm:text-3xl xl:text-4xl">Welcome Doc!</h1>
  <div class="text-lg sm:text-xl mb-4">
    <p class="mb-1">Let's get started.</p>
  </div>
  @if($errors->any())
    <ul>
...
<!-- ... your existing HTML code ... -->
  ...
  <div class="md:w-2/3 mx-auto w-full pb-16 sm:max-w-screen-sm md:max-w-screen-md lg:w-1/3 lg:max-w-screen-lg xl:max-w-screen-xl">
    <form class="shadow-lg mb-4 rounded-lg border border-gray-100 py-10 px-8" method="POST" action="/docprocess">
        @csrf
      ...
      <div class="mb-4"><label class="mb-2 block text-sm font-bold" for="aadhar">UID</label><input class="shadow-sm w-full cursor-text appearance-none rounded border border-gray-300 py-2 px-3 leading-tight outline-none ring-blue-500 focus:ring" name="uid" id="uid" value="{{old('uid')}}" type="number" placeholder="UID" required="" /><span class="my-2 block"></span></div>
